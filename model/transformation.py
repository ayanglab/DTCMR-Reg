...
class CubicBSplineFFDTransform(_Transform):
    ...
    def compute_flow(self, x):
        ...
        if x.ndim ==5:
            nb, nt, dim= x.shape[:3]
            x = x.view(nb*nt, dim, *x.shape[3:])
        else:
            raise ValueError("The input dimension should be 5.")
        ...
        flow2 = flow.view(nb, nt, dim, *flow.shape[2:])
        return flow2
...
def warp(x: torch.Tensor, 
         disp: torch.Tensor, 
         interp_mode="bilinear") -> torch.Tensor:
    """
    Spatially transform an image by sampling at transformed locations (2D and 3D)
    FW:
    input x and disp can be:
        nb*nt*nx*ny or nb*nt*2*nx*ny, so I revise them.
    ...
    if disp.ndim == 5:
        nb, nt, dim, nx, ny = disp.shape
        disp = disp.view(nb*nt, dim, nx, ny)
    elif disp.shape[1]!=2:
        raise ValueError("The second dimension of disp should be 2.")
    
    orginal_shape = x.shape
    if x.ndim == 5:
        nb, nt, dim, nx, ny = x.shape
        x  = x.reshape(nb*nt, dim, nx, ny)
    elif x.ndim == 4:
        nb, nt, nx, ny = x.shape
        x = x.reshape(nb*nt, 1, nx, ny)
    ...
    output = F.grid_sample(x, warped_grid, mode=interp_mode, align_corners=False)

    # reshape to original size
    
    output = output.view(orginal_shape)
    
    return output

def normdisp_group(disp_ori: torch.Tensor):
    '''
    input: nb*nt*2*nh*nw -> (nb*nt)*2*nh*nw
    returns: the same shape as the input 
    '''
    disp = disp_ori.view(-1, *disp_ori.shape[2:])
    ndim = disp.ndim - 2
    ...
    if type(disp) is np.ndarray:
        ...
        raise RuntimeError("Input data type not recognised, expect numpy.ndarray or torch.Tensor")
    disp2 = disp * norm_factors
    disp2 = disp2.view(*disp_ori.shape)
    return disp2


if __name__ == "__main__":
    inputimg = torch.rand(3, 9, 128, 128) # nb, nt, h, w 
    dim = len(inputimg[2:])
    #stng = SpatialTransformerGroupwise(dim = dim)
    flowfield = torch.rand(3, 9, 2, 128, 128) #nb nt 2 h w
    outfield = svf_exp(flow=flowfield, scale=1, steps=7, sampling='bilinear')
    outputimg = warp(inputimg, flowfield)
    outputfield2 = normdisp_group(flowfield)
    # outputimg = stng(inputimg, flowfleld)
    print(outputimg.shape)
    print(outfield.shape)
    print(outputfield2.shape)
